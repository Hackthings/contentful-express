 ...
 *
 */
var renderEntry = function ( id, dir, json ) {
    return template.render( id, "entry", json ).then(function ( html ) {
        var sub = path.join( dir, json.entry.sys.id );

        fs.mkdirSync( sub );
        fs.writeFileSync( path.join( sub, "index.html" ), html );

        lager( "template", "Created build file for build/" + id + "/" + json.entry.sys.id + "/index.html" );
    });
};
...
var renderEntries = function ( id, dir, json ) {
    return template.render( id, "entries", json ).then(function ( html ) {
...
var renderStaticPage = function ( page ) {
    return template.render( "", page, db.getTemplateData() ).then(function ( html ) {
        if ( page === "index" ) {
            ...
            lager( "template", "Created build file for build/" + page + "/index.html" );
        }
    });
};
...
var build = function () {
    return new Promise(function ( resolve, reject ) {
        ...
        staticPages.forEach(function ( page ) {
            promises.push( renderStaticPage( page ) );
        });

        // @build - content types
        for ( var id in contentTypes ) {
            var entries = contentTypes[ id ];
            ...
            promises.push( renderEntries( id, dir, json ) );

            entries.forEach(function ( entry ) {
                var json = db.getTemplateData();
                    json.entry = entry;

                promises.push( renderEntry( id, dir, json ) );
            });
        }
